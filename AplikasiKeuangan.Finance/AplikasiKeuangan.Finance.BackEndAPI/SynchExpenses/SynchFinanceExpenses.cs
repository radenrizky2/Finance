...
namespace AplikasiKeuangan.Finance.BackEndAPI.SynchExpenses
{
    public static class SynchFinanceExpenses
    {
        [FunctionName("SynchFinanceExpenses")]
        public static async Task Run([EventHubTrigger("evh-aplikasi-keuangan",
            Connection = "evh-aplikasi-keuangan", ConsumerGroup ="report.expenses-synchexpenses")]
        EventData[] events, ILogger log)
        {
            ...
            foreach (EventData eventData in events)
            {
                try
                {
                    ...
                    log.LogInformation($"Event body expenses: {eventData.EventBody}");
                    ...
                    foreach (var item in evgData)
                    {
                        ...
                        //        log.LogInformation($"Expenses has been created: {data}");
                        //        break;
                        //    case "Update/":
                        //        var updatedData = JsonConvert.DeserializeObject<DAL.Model.Expenses>(item.Data.ToString());
                        //        log.LogInformation($"Expenses has been updated: {updatedData}");
                        //        break;
                        //    case "Delete/":
                        //        log.LogInformation("Expenses has been deleted");
                        //        break;
